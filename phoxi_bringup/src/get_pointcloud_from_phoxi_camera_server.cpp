...
GetPointCloudFromPhoXiCameraServer::GetPointCloudFromPhoXiCameraServer(ros::NodeHandle& nh) : nh_(nh), is_ok_(false)
{
  ...
  get_pointcloud_server_ = nh.advertiseService("/phoxi_bringup/get_pointcloud", &GetPointCloudFromPhoXiCameraServer::getPointCloud, this);
  start_publish_pointcloud_client_ = nh.serviceClient<phoxi_camera_srvs::PublishPointCloud>("/phoxi_bringup/start_publish_pointcloud");
  stop_publish_pointcloud_client_ = nh.serviceClient<phoxi_camera_srvs::PublishPointCloud>("/phoxi_bringup/stop_publish_pointcloud");
  pointcloud_pub_ = nh.advertise<sensor_msgs::PointCloud2>(cloud_name_, 1);
  pointcloud_sub_ = nh.subscribe("/" + sensor_name_ + "/pointcloud", 1, &GetPointCloudFromPhoXiCameraServer::receivePointCloudCallback, this);
...
bool GetPointCloudFromPhoXiCameraServer::getPointCloud(phoxi_camera_srvs::GetPhoXiPointCloud::Request& req, phoxi_camera_srvs::GetPhoXiPointCloud::Response& res)
{
  is_ok_ = false;
  sensor_msgs::PointCloud2 empty_cloud;
  sensor_cloud_ = empty_cloud;
